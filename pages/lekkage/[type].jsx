...
export default function LekkageType({ type }) {
  ...
  const topSteden = steden.slice(0, 16)

  const faqs = [
    { v: `Wat zijn de oorzaken van een ${type.naam.toLowerCase()}?`, a: type.oorzaken.join(', ') + '. Een van onze specialisten bepaalt ter plaatse de exacte oorzaak.' },
    { v: 'Hoe snel kunnen jullie komen?', a: 'We streven ernaar zo snel mogelijk bij je te zijn. Gemiddeld zijn we binnen 30 minuten ter plaatse. Bel ons voor een actuele inschatting.' },
    { v: 'Vergoedt mijn verzekering dit?', a: 'Plotselinge lekkages zijn bij de meeste opstalverzekeringen gedekt. Wij helpen je met de benodigde documentatie voor je claim.' },
    { v: 'Wat kost een reparatie?', a: 'We werken met transparante tarieven. Je ontvangt altijd een duidelijke prijsopgave voordat we beginnen — geen verborgen kosten.' },
  ]
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`${type.naam} in huis? ${type.omschrijving} LekkageFix helpt snel en vakkundig. Bel direct voor een afspraak.`} />
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .urgency-badge{display:inline-flex;align-items:center;gap:0.4rem;padding:0.3rem 0.85rem;border-radius:100px;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:1rem;background:rgba(232,82,10,0.25);border:1px solid rgba(232,82,10,0.4);color:#ffb494}
        ...
        .oorzaken-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-top:2rem}
        .oorzaak{background:white;border:1.5px solid var(--border);border-radius:12px;padding:1.25rem;display:flex;align-items:flex-start;gap:0.85rem}
        .oorzaak-icon{width:36px;height:36px;background:var(--green3);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0}
        .oorzaak p{font-size:0.83rem;color:var(--muted);line-height:1.5}
        .steden-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(175px,1fr));gap:0.65rem}
        .stad-a{display:flex;align-items:center;justify-content:space-between;background:white;border:1.5px solid var(--border);border-radius:10px;padding:0.75rem 1rem;color:var(--text);text-decoration:none;font-size:0.83rem;font-weight:500;transition:all 0.2s}
        .stad-a:hover{border-color:var(--green);color:var(--green-dark);background:var(--green3)}
        .stad-arrow{color:var(--green);font-size:0.75rem}
        .related-types{display:grid;grid-template-columns:repeat(auto-fill,minmax(175px,1fr));gap:0.65rem}
        ...
        @media(max-width:900px){.hero-inner{grid-template-columns:1fr}.faq-grid{grid-template-columns:1fr}.nav-link{display:none}}
      ...
      <section className="hero">
        <div className="hero-inner">
          <div>
            <div className="urgency-badge">⚠️ {type.urgentie === 'hoog' ? 'Spoed aanbevolen' : 'Tijdig handelen'}</div>
            <h1>{type.icon} <em>{type.naam}</em><br/>verholpen</h1>
          ...
          <div className="form-card" id="offerte">
            ...
            <button className="btn-form">Stuur aanvraag →</button>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="section-inner">
          <div style={{marginBottom:'2rem'}}>
            ...
            <h2>Waardoor ontstaat een <em>{type.naam.toLowerCase()}</em>?</h2>
            <p className="sec-sub">Kennis van de oorzaak is de eerste stap naar een blijvende oplossing.</p>
          </div>
          <div className="oorzaken-grid">
            {type.oorzaken.map((o, i) => (
              <div key={i} className="oorzaak">
                <div className="oorzaak-icon">{['🔍','⚙️','🌧️','🏚️','⏰'][i % 5]}</div>
                <p>{o}</p>
              </div>
            ))}
          </div>
      ...
      <section className="section section-alt" id="steden">
        <div className="section-inner">
          <div style={{marginBottom:'2rem'}}>
            <div className="eyebrow">Per stad</div>
            <h2>{type.naam} in <em>jouw stad</em></h2>
            <p className="sec-sub">Selecteer jouw stad voor lokale informatie en direct een aanvraag indienen.</p>
          </div>
          <div className="steden-grid">
            {topSteden.map(s => (
              <a key={s.slug} href={`/lekkage/${type.slug}/${s.slug}`} className="stad-a">
                📍 {s.naam} <span className="stad-arrow">→</span>
              </a>
            ))}
            <a href="/alle-steden" className="stad-a" style={{color:'var(--green)',fontWeight:600}}>
              Meer steden <span className="stad-arrow">→</span>
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="section section-alt">
        <div className="section-inner">
          <div style={{textAlign:'center',marginBottom:'2rem'}}>
            ...
            <h2>Vragen over <em>{type.naam.toLowerCase()}</em></h2>
          </div>
      ...
      <section className="section">
        <div className="section-inner">
          ...
          <div className="related-types">
            {lekkageTypes.filter(t => t.slug !== type.slug).map(t => (
              <a key={t.slug} href={`/lekkage/${t.slug}`} className="type-a">
                {t.icon} {t.naam}
              </a>
            ))}
          </div>
      ...
      <div className="bottom-cta">
        <h2 style={{color:'white'}}>{type.naam}? Wacht niet af.</h2>
        <p>Hoe eerder je belt, hoe kleiner de schade en de herstelkosten.</p>
