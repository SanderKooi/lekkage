...
export async function getStaticPaths() {
  ...
  lekkageTypes.forEach(type => {
    steden.forEach(stad => {
      paths.push({ params: { stad: stad.slug, type: type.slug } })
    })
  })
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://lekkagefix.nl/lekkage/${stad.slug}/${type.slug}`} />
      ...
      <div className="breadcrumb-bar">
        <div className="breadcrumb">
          ...
          <a href={`/lekkage/${stad.slug}`}>{stad.naam}</a><span className="breadcrumb-sep">›</span>
          <span>{type.naam}</span>
        </div>
      </div>
      ...
      <footer>
        <div className="footer-top">
          ...
          <div className="footer-col">
            ...
            {andereSteden.slice(0,5).map(s => <a key={s.slug} href={`/lekkage/${s.slug}/${type.slug}`}>{s.naam}</a>)}
            <a href={`/lekkage/${stad.slug}`}>Alle steden →</a>
          </div>
          <div className="footer-col">
            <h4>Contact</h4>
